...
struct Location: Decodable {
    let city: String
    let country: String
    let postcode: Postcode //Int // String
}

enum AppError: Error {
    // if not string or int throw this error
    case missingType
}

// create a custom object to be the datatype of postcode
enum Postcode: Decodable {
    // handle two cases of postcode property
    // where postcode
    
    // we will also use associate values to capture the value of postcode
    //
    case int(Int)
    case string(String)
    
    // override init(from decoder:) initializer
    // override decoder when initializing, doing the decoding
    init(from decoder: Decoder) throws {
        // handle the two cases of int or string
        // if it's not an int or a string
        ...
        throw AppError.missingType // was not int or string
    }
}
